...
def _company_priority(token: str) -> int:
    """
    返回这个 token 作为 company 的优先级:
      100 = 中文 ≥2 字符 (高质量匹配)
      50  = 字母 ≥3 含至少 1 大写 (Minimax / BABA)
      10  = 字母 ≥3 全小写 (minimax / deepseek)
      0   = 不像 company,跳过
    """
    if not token:
        return 0
...
def parse_tag(text: str) -> dict:
    ...
    """
    result = {"type": None, "source": None, "company": None,
    ...
    # 6. 剩下的 token(除 date 和 company)拼 title
...
def build_renamed_filename(
    ...
    if user_title:
        ...
        # (例:date=2026-05-07, auto_title=2026-05-07_Acecamp_xxx → Acecamp_xxx)
        if auto:
